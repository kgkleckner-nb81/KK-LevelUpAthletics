-- Level Up Athletics — add Dugout Disaster to the arcade XP allowlist.
--
-- award_arcade_xp() (0017_arcade_game_xp_server_side.sql, widened for
-- Cannon Arm in 0023_cannon_arm_xp.sql) whitelists valid game ids
-- server-side so a tampered client call can't award XP under a made-up
-- game name. Dugout Disaster is a new 5th arcade mini-game (embedded the
-- same iframe/postMessage way as the others, but a self-contained
-- vanilla-JS/Canvas build with no build step) so it needs to be added to
-- that allowlist. Everything else about the function (the 25/day
-- cross-game cap, the 0-25 per-request ceiling) is unchanged.
...
begin
  ...
  if p_game_id not in ('homeRunHero','webGem','clutchCatch','strikeZone','cannonArm','dugoutDisaster') then
